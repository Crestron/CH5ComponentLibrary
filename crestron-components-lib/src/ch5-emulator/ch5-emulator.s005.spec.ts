...
//
    describe('scenario005#cue(type=number,trigger=120)->action(b:link,set,toggle,pulse;n:link,set,increment,decrement,rcb;s:set:link)', () => {
        ...
        it('s005_a6_n_link check',() => {
            if (null !== sigAction6){
                expect(sigAction6.value,'s005_a6_n_link').to.be.equal(0); // fails conversion from string 'up' to number and defaults to 0
            }
        });
        it('s005_a7_n_increment check',() => {
            if (null !== sigAction7) {
                expect(sigAction7.value,'s005_a7_n_increment').to.be.equal(1); // inc 1
            }
        });
        it('s005_a8_n_increment_offset check',() => {
            if (null !== sigAction8){
                expect(sigAction8.value,'s005_a8_n_increment_offset').to.be.equal(10); // inc 10
            }
        });
        it('s005_a9_n_decrement check',() => {
            if (null !== sigAction9){
                expect(sigAction9.value,'s005_a9_n_decrement').to.be.equal(-1); // dec 1
            }
        });
        it('s005_a10_n_decrement_offset check',() => {
            if (null !== sigAction10){
                expect(sigAction10.value,'s005_a10_n_decrement_offset').to.be.equal(-11); // dec 11
            }
        });
        it('s005_a11_n_rcb check before timeout',() => {
            if (null !== sigAction11){
                expect(sigAction11.value,'s005_a11_n_rcb').to.be.equal(0); // test before 100 ms
            }
        });
        it('s005_a11_n_rcb check after timeout',() => {
            if (null !== sigAction11){
                setTimeout(() => {
                    expect(sigAction11.value,'s005_a11_n_rcb').to.be.equal(1234); // test after 100 ms
                },110);
            }
        });
        ...
        it('s005_a7_n_increment recheck',() => {
            if (null !== sigAction7) {
                expect(sigAction7.value,'s005_a7_n_increment').to.be.equal(2); // inc 1
            }
        });
        it('s005_a8_n_increment_offset recheck',() => {
            if (null !== sigAction8){
                expect(sigAction8.value,'s005_a8_n_increment_offset').to.be.equal(20); // inc 10
            }
        });
        it('s005_a9_n_decrement recheck',() => {
            if (null !== sigAction9){
                expect(sigAction9.value,'s005_a9_n_decrement').to.be.equal(-2); // dec 1
            }
        });
        it('s005_a10_n_decrement_offset recheck',() => {
            if (null !== sigAction10){
                expect(sigAction10.value,'s005_a10_n_decrement_offset').to.be.equal(-22); // dec 11
            }
        });
    });
