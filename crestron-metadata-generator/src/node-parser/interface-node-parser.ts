...
export class InterfaceNodeParser implements SubNodeParser {
    ...
    public createType(node: ts.InterfaceDeclaration, typeName: string, context: Context): BaseType {
        if (node.typeParameters && node.typeParameters.length) {
            node.typeParameters.forEach((typeParam) => {
                ...
                if (typeParam.default) {
                    const type = this.childNodeParser.createType(typeParam.default, typeName, context);
        ...
        return new ObjectType(
            ...
            this.getBaseTypes(node, typeName, context),
            this.getProperties(node, typeName, context),
            this.getAdditionalProperties(node, typeName, context),
        );
    }

    private getBaseTypes(node: ts.InterfaceDeclaration, typeName: string, context: Context): BaseType[] {
        ...
        return node.heritageClauses.reduce((result: BaseType[], baseType) => [
            ...result,
            ...baseType.types.map((expression) => this.childNodeParser.createType(expression, typeName, context)),
        ], []);
    }

    private getProperties(node: ts.InterfaceDeclaration, typeName: string, context: Context): ObjectProperty[] {
        return node.members
    ...
    private getAdditionalProperties(node: ts.InterfaceDeclaration, typeName: string, context: Context): BaseType | false {
