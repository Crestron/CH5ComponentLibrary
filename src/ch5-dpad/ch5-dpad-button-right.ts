...
import { IRightBtnContract } from "./interfaces/i-ch5-dpad-utils";
...
export class Ch5DpadRight extends Ch5Common implements ICh5DpadRightAttributes {
    ...
    private buildParentBasedContract() {
        const contract: IRightBtnContract = CH5DpadContractUtils.getRightBtnContract();
        ...
        if (Boolean(contractName)) {
            CH5DpadContractUtils.checkAndUpdateClickHandler(this, `${contractName}.${contract.RightClicked}`);
            if (show) {
                CH5DpadContractUtils.checkAndUpdateShowHandler(this, `${contractName}.${contract.RightShow}`);
            }
            if (enable) {
                CH5DpadContractUtils.checkAndUpdateEnableHandler(this, `${contractName}.${contract.RightEnable}`);
            }
            if (icon) {
                CH5DpadContractUtils.checkAndUpdateIconClassHandler(this, `${contractName}.${contract.RightIconClass}`);
                CH5DpadContractUtils.checkAndUpdateIconUrlHandler(this, `${contractName}.${contract.RightIconUrl}`);
            }
        }
    }
