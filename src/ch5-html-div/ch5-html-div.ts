...
import { ICh5HtmlDivAttributes } from './interfaces/i-ch5-html-div-attributes';
import { Ch5Properties } from "../ch5-core/ch5-properties";
import { ICh5PropertySettings } from "../ch5-core/ch5-property";

export class Ch5HtmlDiv extends Ch5Common implements ICh5HtmlDivAttributes {
	...
	public static readonly ELEMENT_NAME = 'ch5-html-div';

	public primaryCssClass = 'ch5-html-div';
	...
	public static registerSignalAttributeTypes() {
		Ch5SignalAttributeRegistry.instance.addElementAttributeEntries(Ch5HtmlDiv.ELEMENT_NAME, Ch5HtmlDiv.SIGNAL_ATTRIBUTE_TYPES);
	}
	...
	public static registerCustomElement() {
		if (typeof window === "object"
			...
			&& window.customElements.get(Ch5HtmlDiv.ELEMENT_NAME) === undefined) {
			window.customElements.define(Ch5HtmlDiv.ELEMENT_NAME, Ch5HtmlDiv);
		}
	}
	...
	public static get observedAttributes(): string[] {
		...
		for (let i: number = 0; i < Ch5HtmlDiv.COMPONENT_PROPERTIES.length; i++) {
			if (Ch5HtmlDiv.COMPONENT_PROPERTIES[i].isObservableProperty === true) {
				newObsAttrs.push(Ch5HtmlDiv.COMPONENT_PROPERTIES[i].name.toLowerCase());
			}
		}
	...
	public attributeChangedCallback(attr: string, oldValue: string, newValue: string): void {
		...
		if (oldValue !== newValue) {
			this.logger.log('ch5-html-div attributeChangedCallback("' + attr + '","' + oldValue + '","' + newValue + '")');
			const attributeChangedProperty = Ch5HtmlDiv.COMPONENT_PROPERTIES.find((property: ICh5PropertySettings) => { return property.name.toLowerCase() === attr.toLowerCase() && property.isObservableProperty === true });
	...
	/**
	 * Called when the Ch5HtmlDiv component is first connected to the DOM
	 */
	public connectedCallback() {
		this.logger.start('connectedCallback()', Ch5HtmlDiv.ELEMENT_NAME);
		// WAI-ARIA Attributes
	...
	protected initAttributes() {
		...
		for (let i: number = 0; i < Ch5HtmlDiv.COMPONENT_PROPERTIES.length; i++) {
			if (Ch5HtmlDiv.COMPONENT_PROPERTIES[i].isObservableProperty === true) {
				if (this.hasAttribute(Ch5HtmlDiv.COMPONENT_PROPERTIES[i].name.toLowerCase())) {
					const key = Ch5HtmlDiv.COMPONENT_PROPERTIES[i].name;
...
Ch5HtmlDiv.registerCustomElement();
Ch5HtmlDiv.registerSignalAttributeTypes();
